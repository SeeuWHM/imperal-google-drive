...
[![Version](https://img.shields.io/badge/version-0.3.5-green)](https://github.com/SeeuWHM/imperal-google-drive/releases)
...
Or use the panel — split across two focused surfaces:
- **Left sidebar** (`doc_overview`) — connected Google accounts at a glance (switch/disconnect inline), file-count per account.
- **Right panel** (`doc_files`) — the active account's files: a storage bar against the quota, Pick files, and a searchable, **multi-select** list (checkboxes, like ctrl-click) with a bulk toolbar (Remove / Retry indexing). A one-click "Remove all N folders" button also appears when there is more than one picked folder — since picked folders are not actually readable (see Known limitations), a pile of them (e.g. a dozen "Debug" build folders) is pure clutter, and clearing them shouldn't need a manual multi-select of every duplicate.
...
| **file_bulk_action** | Panel-only — multi-select toolbar dispatcher (remove / retry indexing), one call regardless of selection size |
...
├── panels.py              # Right-slot panel (doc_files) — storage bar, toolbar, multi-select files/folders list + bulk actions
├── panels_overview.py     # Left-slot panel (doc_overview) — connected accounts (switch/disconnect), at a glance
...
├── handlers_bulk.py       # file_bulk_action — panel-only multi-select bulk dispatcher (remove / retry_index)
...
| `file_bulk_action` | write (panel-only) | Multi-select bulk remove / retry-index — one call, not one per file |
